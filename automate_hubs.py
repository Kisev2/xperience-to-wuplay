...
def build_catalog_entry(source, catalog_map, addon_id, transport_url):
    ...
    for ex in meta.get("extra", []):
        ...
        val = source.get(name)
        if val is None:
            if ex.get("isRequired") and ex.get("options"):
                val = ex["options"][0]
        if val is not None:
            extra_payload.append({"name": name, "value": str(val)})
...
def main():
    ...
    catalog_map = {}
    try:
        print("  Fetching live manifest automatically...")
        manifest_url = f"{transport_url}/manifest.json"
        ...
        print(f"  Successfully loaded {len(catalog_map)} live catalog definitions")
    except Exception as e:
        print(f"  Warning: Failed to fetch live manifest ({e}), falling back to local file...")
        catalog_map = load_manifest_catalog_map(manifest_file)
        print(f"  Loaded {len(catalog_map)} local catalog definitions")
        
    print(f"  {len(collections)} collections loaded")
